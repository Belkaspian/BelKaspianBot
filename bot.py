...
==================================================
- Очистка телефонов: удалять пробелы, скобки, тире; оставлять только цифры и ведущий '+'. Возвращать только уникальные номера (без дублей).
  Сортировку по приоритету (+7, +375) выполняет бэкенд.
- Очистка номеров документов (паспорт, ВУ): удалять символы "№" и внутренние пробелы между серией и номером.
- Очистка VIN: удалять пробелы и дефисы перед валидацией длины.
- Нормализация VIN символов: 'I'→'1', 'O'→'0', 'Q'→'0' (только в VIN). Если после нормализации длина = 17 — VIN считается валидным.
- Конвертация кодов стран (RUS, BY, KZ, PL и т.д.) в полные названия на русском.
- Приведение дат к формату ДД.ММ.ГГГГ, если дата явно распознаваема.
- Для expiry_date (срока действия): если год указан двумя цифрами, дополнять до четырёх так: 00–40 → 20XX; 41–99 → 19XX. Если контекст неоднозначен — expiry_date = null и добавлять review_reasons "Дата указана только месяц/год или двузначный год неоднозначен".
...
- **Алфавит для plate_number (явно):**
  * Если страна регистрации ТС в {Россия, Беларусь, Казахстан} — для визуально совпадающих графем записывать кириллические символы: А, В, Е, К, М, Н, О, Р, С, Т, У, Х (Unicode кириллица).
  * Если страна регистрации — европейская или явно латинская — записывать латиницей.
  * Если country = null и plate_number состоит только из перечисленных совпадающих графем — записывать эти графемы кириллицей.
  * Не транслитерировать произвольно; применять правило страны.
- country: страна регистрации ТС — полное название на русском или null.
- type: одно из TRUCK, TRAILER, UNKNOWN.
- plate_number и vin принадлежат объекту vehicle.
...
{ 
  ...
  "driver":{"full_name":null,"country":null,"passport":{"series_number":null,"expiry_date":null},"license":{"number":null,"expiry_date":null},"phones":[]},
  "vehicles":[
    {
      "type":"TRUCK",
      "brand":"VOLVO",
